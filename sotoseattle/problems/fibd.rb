...
class Rosalind
  ...
  def self.fibo_kill n, m
    @@cache[n] ||= if n < 0
                     ...
                     (2..m).inject(0) { |t, i| t += fibo_kill(n-i, m) }
                   end
  end
end
...
class TestRosalind < Minitest::Test
  def test_fibonacci_on_steroids
    skip
    assert_equal 1, Rosalind.fibo_kill(1, 3)
    assert_equal 1, Rosalind.fibo_kill(2, 3)
    assert_equal 2, Rosalind.fibo_kill(3, 3)
    assert_equal 2, Rosalind.fibo_kill(4, 3)
    assert_equal 3, Rosalind.fibo_kill(5, 3)
    assert_equal 4, Rosalind.fibo_kill(6, 3)
    assert_equal 12081658148171622250, Rosalind.fibo_kill(93, 17)
  end
end
